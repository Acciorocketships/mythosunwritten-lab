extends SceneTree
## Entry point for the goodwill run: the two ways section 6 says sentiment goes
## up, played as two arms of one world and measured against the ownership rule.
## Exit 0.
##
## Run it with:  ./run_goodwill.sh [--seed N] [--ticks N] [--roll-seed N] [--live]
...
func _parse_args(args: PackedStringArray) -> Dictionary:
	var options := {
		"seed": ScriptedGoodwill.SEED, "ticks": ScriptedGoodwill.TICKS,
		"roll-seed": ScriptedGoodwill.ROLL_SEED, "live": false,
	}
	var i := 0
	while i < args.size():
		var arg := args[i]
