extends SceneTree
## Entry point for the goal comparison: one character, one moment, and the same
## question put with nothing on its goal list and with each of three goals on it.
## Exit 0.
##
## Run it with:  ./run_goal.sh [--live]
...
func _initialize() -> void:
	...
	for arg in args:
		if arg != "--live":
			...
			printerr("usage: run_goal.sh [--live]")
			quit(2)
			return
	var flights := []
