...
func _init() -> void:
	...
	var frame := world.frame_for(Vector2i(-1,-1))
	...
	var g := urban.terrain_grade
	var layers := []
	while g != null:
		layers.append({"claims":g._claims,"continuous":g._continuous_cells,"datum":g._continuous_datum,"origin":g._origin,"pitch":g._targets.pitch})
		g = g._continuous_source
	FileAccess.open("/tmp/september8-west-grade.txt",FileAccess.WRITE).store_string(var_to_str(layers))
	print("GRADE_FRAME ",urban.world_transform," layers=",layers.size())
	print("CONTACTS ",VillageWarrenFabricSolver.terrain_contact_specs(urban.volumetric_spatial,urban.fabric_plan))
	...
	FileAccess.open("res://tests/fixtures/september8-west-source.txt",FileAccess.WRITE).store_string(var_to_str(frozen))
	FileAccess.open("/tmp/september8-west-skin-transaction.txt",FileAccess.WRITE).store_string(var_to_str(SettlementFabricAssembler.maze_ground_skin_transaction(urban.fabric_plan)))
	var file := FileAccess.open("/tmp/village-september8-west-probe.json",FileAccess.WRITE)
