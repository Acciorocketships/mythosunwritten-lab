...
func _init() -> void:
	...
	FileAccess.open("res://tests/fixtures/september7-manual-source.txt",FileAccess.WRITE).store_string(var_to_str(frozen))
	FileAccess.open("/tmp/september7-skin-transaction.txt",FileAccess.WRITE).store_string(var_to_str(SettlementFabricAssembler.maze_ground_skin_transaction(urban.fabric_plan)))
	var file := FileAccess.open("/tmp/village-september7-probe.json",FileAccess.WRITE)
